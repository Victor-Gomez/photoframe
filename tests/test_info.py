...
def test_named_people_are_listed(app, client, exif):
    ...
    with app.db.lock:
        app.db.borrow().execute("INSERT INTO person (id, name) VALUES ('p1', 'Trini')")
        app.db.borrow().execute("INSERT INTO cluster (id, person_id, ci) VALUES ('c1', 'p1', 1)")
        app.db.borrow().execute(
            "INSERT INTO face (rel, idx, cluster_id) VALUES (?, 0, 'c1')", (exif,)
        )
        app.db.borrow().commit()

    info = client.get(f"/api/info/{photo_id_of(exif)}").get_json()
...
def test_a_photo_without_them_omits_them(app, client):
    info = client.get(f"/api/info/{photo_id_of('Trip/Day2/pano.avif')}").get_json()
